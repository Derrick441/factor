...
class trade_module(object):
    ...
    def dateend_rightissue(self,tradedate,curprice,curri_ex_dt,curri_price,curri_ratio):
        ...
        if idx.any():
            ...
            self.stocks[idx] = np.maximum(0,self.stocks[idx]-self.ri_recordinfo_d[idx,0])+np.minimum(self.stocks[idx],self.ri_recordinfo_d[idx,0])/self.ri_recordinfo_d[idx,1]
    ...
    def tradeflow(self):
        ...
        self.tradeinfo = pd.DataFrame(data=None,index=datelist,columns=['cash','exdvdcash','exstockvalue','stockvalue','sellmoney','buymoney','total'])  # 交易信息
...
if __name__ == '__main__':
    ...
    tradem = trade_module(closep,openp,vol,weight,enddate,initmoney,costsell,costbuy,
                                         ex_dt,dvd_payout_dt,listing_dt_of_dvd_shr,stk_dvd_per_sh,cash_dvd_per_sh_after_tax,
                                         ri_ex_dt,ri_price,ri_ratio)
    tradem.tradeflow()
