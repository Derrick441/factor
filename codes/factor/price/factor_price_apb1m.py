...
# 20日均价偏差apb: 20日的成交量加权平均价的平均价/20日的成交量加权的成交量加权平均价的平均价
class Apb1m(object):
    ...
    def month_apb(self, data):
        temp = data.copy()
        # 计算20日的日成交量加权平均价的平均价
        temp['vwap_mean'] = temp['vwap'].rolling(20).mean()
        # 计算20日的成交量加权的日成交量加权平均价的平均价
        temp['volume_20'] = temp['s_dq_volume'].rolling(20).sum()
        temp['temp'] = temp['s_dq_volume'] / temp['volume_20'] * temp['vwap']
        temp['vwap_mean_vw'] = temp['temp'].rolling(20).sum()
        # 计算apb
        temp['apb1m'] = np.log(temp['vwap_mean'] / temp['vwap_mean_vw'])
        return temp[['trade_dt', 'apb1m']]
...
if __name__ == '__main__':
    ...
    apb = Apb1m(file_indir, file_name)
    apb.runflow()
