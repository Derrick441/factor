...
class FinanceReportGen(object):
    def __init__(self,INDEX,indir,ftable,factornames,keep_report,enddate,longtimeago,QuarterNum):
        self.INDEX = INDEX
        self.indir = indir
        self.ftable = ftable
    ...
    def sqlGetName(self,conn):
        sqlq = "select COLUMN_NAME from all_tab_columns where owner='WIND' and table_name='"+self.ftable+"'"
        factornames = pd.read_sql(sqlq,conn)
        return factornames['COLUMN_NAME'].apply(lambda x:x.lower())

    def sqlDropName(self,factornames):
        dropfactor = ['s_info_windcode','wind_code','ann_dt','report_period','crncy_code','object_id','opdate','opmode']
        factornames = factornames[~factornames.isin(dropfactor)]
        return factornames
    ...
    def sqlIn(self):
        ...
        where report_period>'"+self.longtimeago+"' and statement_type in ('408001000','408004000') and \
        s_info_windcode<='699999.SH' and ann_dt<='"+self.enddate+"' order by s_info_windcode,report_period,ann_dt"
    ...
    def seasonShift(self):
        ...
        # 每一季财报，其需要用到当年(year0，作为当年财报),一年后(year1，作为去年财报),year2,year3,year4 等5个日期里面
        # 此处生成这5个日期
    ...
    def seasonMapSub(self,setyear,setyearp1,setseason,setname):
        ...
        # 删除冗余数据
        datef.drop(labels=['reportyear','reportseason','listdate','year+0','year+1','year+2','year+3','year+4'],
                   axis=1,inplace=True)
    ...
    def dataSeperateYearOut(self,factor,name):
        ...
        for year in range(int(startdate[0:4]),int(self.enddate[0:4])+1):
            ...
            yf.drop(['trade_year'],axis=1).to_pickle(self.indir+self.INDEX+'/'+self.INDEX+'_'+
                                                     self.ftable+name+'_'+str(year)+'.pkl')
...
if __name__ == '__main__':
    ...
    QuarterNum = 12
    factornames = ['statement_type','oper_rev']
    financereport = FinanceReportGen(INDEX,indir,ftable,factornames,keep_report,enddate,longtimeago,QuarterNum)
