...
class FactorMomx(object):
    ...
    def compute(self):
        ...
        # 滚动累加
        self.data_mom['mom1_5'] = self.data_mom.groupby('s_info_windcode')['mom1_temp'].rolling(5).sum().values
        self.data_mom['mom2_5'] = self.data_mom.groupby('s_info_windcode')['mom2_temp'].rolling(5).sum().values
        self.data_mom['mom3_5'] = self.data_mom.groupby('s_info_windcode')['mom3_temp'].rolling(5).sum().values
        self.data_mom['mom4_5'] = self.data_mom.groupby('s_info_windcode')['mom4_temp'].rolling(5).sum().values
        print('compute using time:%10.4fs' % (time.time() - t))

    def fileout(self):
        t = time.time()
        item = ['trade_dt', 's_info_windcode', 'mom1_5']
        self.data_mom[item].to_pickle(self.save_indir + 'factor_hq_mom1_5_' + self.file_name[17:21] + '.pkl')

        item = ['trade_dt', 's_info_windcode', 'mom2_5']
        self.data_mom[item].to_pickle(self.save_indir + 'factor_hq_mom2_5_' + self.file_name[17:21] + '.pkl')

        item = ['trade_dt', 's_info_windcode', 'mom3_5']
        self.data_mom[item].to_pickle(self.save_indir + 'factor_hq_mom3_5_' + self.file_name[17:21] + '.pkl')

        item = ['trade_dt', 's_info_windcode', 'mom4_5']
        self.data_mom[item].to_pickle(self.save_indir + 'factor_hq_mom4_5_' + self.file_name[17:21] + '.pkl')
