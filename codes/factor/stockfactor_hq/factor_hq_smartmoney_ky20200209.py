...
class FactorSmartMoney(object):
    ...
    def method(self, data, perid):
        ...
        if num >= perid:
            ...
            for i in range(x, num):
                ...
                temp_data = temp_part.sort_values(by='S1', ascending=False).copy()
            ...
            return pd.DataFrame({'trade_dt': date,
                                 'smartmoney': [None for i in range(num)],
                                 'smartmoneyln': [None for i in range(num)]})
    ...
    def fileout(self):
        t = time.time()
        item1 = ['trade_dt', 's_info_windcode', 'smartmoney']
        self.result[item1].to_pickle(self.save_indir + 'factor_hq_smartmoney_' + self.file_name[17:21] + '.pkl')

        item2 = ['trade_dt', 's_info_windcode', 'smartmoneyln']
        self.result[item2].to_pickle(self.save_indir + 'factor_hq_smartmoneyln_' + self.file_name[17:21] + '.pkl')
...
if __name__ == '__main__':
    ...
    for file_name in file_names:
        fsm = FactorSmartMoney(file_indir, save_indir, file_name)
        fsm.runflow()
