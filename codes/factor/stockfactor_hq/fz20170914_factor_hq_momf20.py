...
class FactorMomf20(object):
    ...
    def compute(self):
        ...
        self.data_dropna['momf20'] = np.dot(self.data_dropna[item], self.weight)
        print('compute running time:%10.4fs' % (time.time() - t))

    def fileout(self):
        t = time.time()
        self.result = pd.merge(self.data_mom0[['trade_dt', 's_info_windcode']], self.data_dropna, how='left')
        item = ['trade_dt', 's_info_windcode', 'momf20']
        self.result[item].to_pickle(self.save_indir + 'factor_hq_momf20.pkl')
...
if __name__ == '__main__':
    ...
    momf20 = FactorMomf20(file_indir, factor_indir, save_indir, file_name, factor_names, method)
    momf20.runflow()
