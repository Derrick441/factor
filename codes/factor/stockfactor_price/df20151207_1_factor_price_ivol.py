...
class Ivol(object):
    ...
    def method(self, data, perid):
        ...
        if num >= perid:
            ...
            print(time.time() - t)
            return pd.DataFrame({'trade_dt': temp.trade_dt.values, 'ivol': temp_result.values})
        else:
            return pd.DataFrame({'trade_dt': temp.trade_dt.values, 'ivol': [None for i in range(num)]})
    ...
    def fileout(self):
        ...
        item = ['trade_dt', 's_info_windcode', 'ivol']
        self.result[item].to_pickle(self.save_indir + 'factor_price_ivol.pkl')
...
if __name__ == '__main__':
    ...
    ivol = Ivol(file_indir, save_indir, file_name)
    ivol.runflow()
