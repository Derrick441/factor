...
class ApbN(object):
    ...
    def method(self, data, index, weight, perid, name):
        temp = data.copy()
        num = len(temp)
        if num >= perid:
    ...
    def compute(self):
        ...
        self.temp_result = self.data_dropna.groupby('s_info_windcode')\
                                           .apply(self.method, 'vwap', 's_dq_volume', 20, 'apb20d')\
                                           .reset_index()
        print('compute running time:%10.4fs' % (time.time() - t))

    def fileout(self):
        t = time.time()
        self.result_20 = pd.merge(self.all_data[['trade_dt', 's_info_windcode']], self.temp_result, how='left')
...
if __name__ == '__main__':
    ...
    apbn = ApbN(file_indir, save_indir, file_name)
    apbn.runflow()
