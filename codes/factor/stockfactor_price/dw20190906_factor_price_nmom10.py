...
class FactorNmom10(object):
    ...
    def compute(self):
        ...
        self.temp_meanstd = self.data_dropna.groupby('trade_dt')\
                                            ...
                                            .apply(pd.Series)\
                                            .reset_index()\
                                            .rename(columns={0: 'momi10_mean', 1: 'momi10_std',
                                                             2: 'momo10_mean', 3: 'momo10_std'})
        self.temp_result = pd.merge(self.all_data, self.temp_meanstd, how='left')
        self.temp_result['nmomi10'] = ((self.temp_result.momi10-self.temp_result.momi10_mean)/self.temp_result.momi10_std)
        self.temp_result['nmomo10'] = ((self.temp_result.momo10-self.temp_result.momo10_mean)/self.temp_result.momo10_std)
        self.temp_result['nmom10'] = self.temp_result['nmomi10'] + self.temp_result['nmomo10']
        print('compute running time:%10.4fs' % (time.time() - t))

    def fileout(self):
        t = time.time()
        self.result = pd.merge(self.all_data[['trade_dt', 's_info_windcode']], self.temp_result, how='left')
        item = ['trade_dt', 's_info_windcode', 'nmom10']
        self.result[item].to_pickle(self.save_indir + 'factor_price_nmom10.pkl')
...
if __name__ == '__main__':
    ...
    file_name = ['factor_price_momi10.pkl', 'factor_price_momo10.pkl']

    nmom10 = FactorNmom10(file_indir, save_indir, file_name)
    nmom10.runflow()
