...
class Factormomi5(object):
    ...
    def compute(self):
        ...
        self.intarday_data = self.data_dropna.groupby('s_info_windcode')\
                                             .apply(self.method, 5, item[0], item[1])\
                                             .reset_index()
        # 横截面均值、标准差
        item = ['part1', 'part5']
        ...
        self.temp_result['momi5'] = self.temp_result['t5'] - self.temp_result['t1']
        print('compute running time:%10.4fs' % (time.time() - t))

    def fileout(self):
        t = time.time()
        self.result = pd.merge(self.all_data[['trade_dt', 's_info_windcode']], self.temp_result, how='left')
        item = ['trade_dt', 's_info_windcode', 'momi5']
        self.result[item].to_pickle(self.save_indir + 'factor_price_momi5.pkl')
...
if __name__ == '__main__':
    ...
    momi5 = Factormomi5(file_indir, save_indir, file_name)
    momi5.runflow()
