...
class FactorMomi(object):
    ...
    def datamanage(self):
        t = time.time()
        self.all_data['tdr'] = self.all_data['s_dq_close'] / self.all_data['s_dq_open'] - 1
    ...
    def method(self, data, perid, index1, index2):
        ...
        if num >= perid:
            ...
            for i in range(perid, num):
                ...
                part1.append(np.mean(temp_data[index2][:4]))
                part5.append(np.mean(temp_data[index2][-4:]))
            print(time.time() - t)
    ...
    def compute(self):
        ...
        # merge
        self.temp_result = pd.merge(self.intarday_data, self.intarday_meanstd, how='left')
        # t值
        ...
        self.temp_result['momi'] = self.temp_result['t5'] - self.temp_result['t1']
        print('compute running time:%10.4fs' % (time.time() - t))

    def fileout(self):
        t = time.time()
        self.result = pd.merge(self.all_data[['trade_dt', 's_info_windcode']], self.temp_result, how='left')
        item = ['trade_dt', 's_info_windcode', 'momi']
        self.result[item].to_pickle(self.save_indir + 'factor_price_momi.pkl')
...
if __name__ == '__main__':
    ...
    momi = FactorMomi(file_indir, save_indir, file_name)
    momi.runflow()
