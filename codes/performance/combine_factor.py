...
class FactorCombine(object):
    ...
    def combine_factor_ic(self, data_ic, data_fa, flag):
        t = time.time()
        # 数据备份
        ...
        ic_i_roll20 = temp_ic.rolling(20).mean()
        ic_i_roll20.reset_index(inplace=True)
        temp_ic_merge = pd.merge(temp_fa[item], ic_i_roll20, how='left')
        # 因子：IC加权、求和得到合并因子
        temp1 = temp_fa.set_index(item)
        temp2 = temp_ic_merge.set_index(item)
        result = pd.DataFrame(np.array(temp1) * np.array(temp2), index=temp1.index)
        result['combine' + flag] = result.sum(axis=1)
        # 0替换为nan、reset_index
        final = pd.DataFrame(result['combine' + flag].replace(0, np.nan))
        final.reset_index(inplace=True)
        final['trade_dt'] = final['trade_dt'].apply(lambda x: str(x))
        print('combine using time:%10.4fs' % (time.time() - t))
        return final

    def compute(self):
        # 合并因子(以ic为权重）
        self.combine_factor_ic_1 = self.combine_factor_ic(self.table_ic_1, self.factor_std, '_ic_1')
        self.combine_factor_ic_5 = self.combine_factor_ic(self.table_ic_5, self.factor_std, '_ic_5')
        self.combine_factor_ic_10 = self.combine_factor_ic(self.table_ic_10, self.factor_std, '_ic_10')
        self.combine_factor_ic_20 = self.combine_factor_ic(self.table_ic_20, self.factor_std, '_ic_20')
        self.combine_factor_ic_60 = self.combine_factor_ic(self.table_ic_60, self.factor_std, '_ic_60')
