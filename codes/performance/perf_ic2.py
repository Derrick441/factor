...
# 计算因子值与收益值之间相关系数（因子和收益都市值、行业中性化）
class PerformanceIc2(object):
    ...
    def compute_ic(self):
        ...
        print('compute_ic(factor mv_indu_neutral) running time:%10.4fs' % (time.time() - t))

        # 收益市值、行业中性化
        self.data['r1_remove'] = self.data.groupby(level=0).apply(self.mv_ind_neutral, 'ret_1', mv_ind_item).values
        self.data['r5_remove'] = self.data.groupby(level=0).apply(self.mv_ind_neutral, 'ret_5', mv_ind_item).values
        self.data['r10_remove'] = self.data.groupby(level=0).apply(self.mv_ind_neutral, 'ret_10', mv_ind_item).values
        self.data['r20_remove'] = self.data.groupby(level=0).apply(self.mv_ind_neutral, 'ret_20', mv_ind_item).values
        self.data['r60_remove'] = self.data.groupby(level=0).apply(self.mv_ind_neutral, 'ret_60', mv_ind_item).values
        print('compute_ic(factor+return mv_indu_neutral) running time:%10.4fs' % (time.time() - t))

        # 计算每期因子ic或rank ic值
        item_ic = [self.factor_name, 'r1_remove', 'r5_remove', 'r10_remove', 'r20_remove', 'r60_remove']
    ...
    def fileout(self):
        ...
        self.result.to_pickle(self.indir_p + self.method + '2_' + self.name_f)
...
#
#     performanceic2 = PerformanceIc2(indir_dataflow, name_dataflow, indir_factor, name_factor, indir_perf, 'IC')
#     performanceic2.runflow()
