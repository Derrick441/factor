...
# 每日for计算
def for_compute(max_time, mode, bl, sta, price_list, date_list, compute_num):
    t = datetime.now()
    # 数据整理
    if mode == 't1':
        y = price_list[sta:]
    ...
    data_lens = pd.Series([len(y)/bl, startday, endday], index=['len', 'startday', 'endday'])
    print('{} {:.2%}'.format(sta, (sta+1)/len(price_list)), datetime.now() - t)
    try:
...
# 遍历计算（前后都可）多进程
def bl_compute_new(data_get, stock, t1, t2, keep_num, max_time, process_num, compute_num, mode, bl, ln, file_dir):
    # SQL参数
    account = 'wind_read'
    password = 'password1'
    ...
    for sta in sta_range:
        result_trav.append(pool.apply_async(for_compute, (max_time, mode, bl, sta, price_list, date_list, compute_num)))
    pool.close()
    pool.join()
    # 参数估计结果汇总
    result_trav_ = []
    ...
    # 崩盘日
    result['breakday_predict'] = result['tc']/bl - result['len']
...
# 作图
def lppls_picture(stock, t1, t2, result_dir, mode, compute_num, danger):
    # 数据读入
    if mode in ('t1', 't2'):
        dir_part = '_' + mode + '_' + t1 + '_' + t2
    ...
    if mode in ('t1', 't2'):
        ...
        plt.legend(l1 + l2, ["breakday_predict", "R2"], loc=4)
    ...
    if mode == 't1':
        ...
        # 多线重叠图(price)
        result_all['price_1'] = result_all[result_all['breakday_predict'] < danger]['price']
        result_all['price_2'] = result_all[result_all['breakday_predict'] >= danger]['price']
        # 作图展示
...
if __name__ == '__main__':
    # 输入信息
    stock, t1, t2, mode, tdelta = input('输入：股票 开始 结束 模式 频率\n').split()
    ...
    t = datetime.now()
    print('\n计算开始:')
    bl_compute_new(data_get, stock, t1, t2, keep_num, max_time, process_num, compute_num, mode, bl, ln, file_dir)
    lppls_picture(stock, t1, t2, file_dir, mode, compute_num, danger)
    print('\n计算结束.', mode, datetime.now() - t)
