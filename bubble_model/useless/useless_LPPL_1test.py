...
# 数据准备
ydata, n = data_in('20140101', '20150501')
xdata = np.linspace(0, n-1, n)
...
data_mses = pd.DataFrame({'date': xdata[:-30]-n+7, 'mses': mses}).dropna()
...
# f_y = ydata[i:]
# n_ = len(f_y)
# f_x = np.linspace(0, n_-1, n_)
# # f_x = xdata[i:]
# bds = ([0, -np.inf, -np.inf, -np.inf, 0, 2, n_], [np.inf, np.inf, np.inf, np.inf, 2, 50, n_*2])
# popt, pcov = curve_fit(LPPL, f_x, f_y, bounds=bds, method='trf')
#
# f1, ax1 = plt.subplots(1, 1, figsize=(6, 3))
# ax1.plot(f_x, f_y, 'b-')
# ax1.plot(f_x, LPPL(f_x, *popt), 'r-')
# plt.show()
