...
# 核心因子计算
@print_execute_time
def ma_keycompute(data, factor_name, factor_type, t1):
    periods = [5, 10, 20, 60, 120, 240]
    factor_names = ['ma005', 'ma010', 'ma020', 'ma060', 'ma120', 'ma240']
    items = zip(periods, factor_names)
    factor_list = []
    for n, name in items:
        # 核心计算
        ma = data.rolling(n).mean() / data
        factors = ma.stack().reset_index().rename(columns={0: 'factor_value'})
        # 补充信息
...
# 因子计算
def ma_compute(data_raw, t1, factor_name, factor_type):
    # 数据处理
    data = ma_datamanage(data_raw, t1)
    # 核心因子计算
    factors = ma_keycompute(data, factor_name, factor_type, t1)
    return factors


# 因子
@print_execute_time
def ma_sql_compute_sql(factor_name, factor_type, t1, t2, save, factor_table):
    # 数据读取
    data_raw = vpd_data_read(t1, t2, 13)
    # 因子计算
    factors = ma_compute(data_raw, t1, factor_name, factor_type)
    # 因子存储
...
if __name__ == '__main__':
    ...
    #     print(t1, t2)
    #     ma_sql_compute_sql('ma', 'vpd', t1, t2, 1, 'fac_vpd_mom')

    fac_test = ma_sql_compute_sql('ma', 'vpd', '20181228', '20181228', 0, 'test')
