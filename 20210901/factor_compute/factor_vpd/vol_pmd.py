...
# 数据处理
@print_execute_time
def pmd_datamanage(data_raw, t1):
    data = data_raw.copy()
    # 停牌:nan
...
# 核心因子计算
@print_execute_time
def pmd_keycompute(data, factor_name, factor_type, t1):
    periods = [5, 10, 20, 60, 120, 240]
    factor_names = ['pmd005', 'pmd010', 'pmd020', 'pmd060', 'pmd120', 'pmd240']
    items = zip(periods, factor_names)
    factor_list = []
    for n, name in items:
        # 核心计算
        mom = data.pct_change(n)
        pctsum = data.pct_change(1).applymap(lambda x: abs(x)).rolling(n).sum()
        pmd = mom / pctsum
        factors = pmd.stack().reset_index().rename(columns={0: 'factor_value'})
        # 补充信息
...
# 因子计算
def pmd_compute(data_raw, t1, factor_name, factor_type):
    # 数据处理
    data = pmd_datamanage(data_raw, t1)
    # 核心因子计算
    factors = pmd_keycompute(data, factor_name, factor_type, t1)
    return factors


# 因子
@print_execute_time
def pmd_sql_compute_sql(factor_name, factor_type, t1, t2, save, factor_table):
    # 数据读取
    data_raw = vpd_data_read(t1, t2, 13)
    # 因子计算
    factors = pmd_compute(data_raw, t1, factor_name, factor_type)
    # 因子存储
...
if __name__ == '__main__':
    ...
    for t1, t2 in time_list:
        print(t1, t2)
        pmd_sql_compute_sql('pmd', 'vpd', t1, t2, 1, 'fac_vpd_vol')

    # fac_test = pmd_sql_compute_sql('pmd', 'vpd', '20181228', '20181228', 0, 'test')
