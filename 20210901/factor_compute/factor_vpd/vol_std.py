...
# 数据处理
@print_execute_time
def std_datamanage(data_raw, t1):
    data = data_raw.copy()
    # 停牌:nan
...
# 核心因子计算
@print_execute_time
def std_keycompute(data, factor_name, factor_type, t1):
    periods = [5, 10, 20, 60, 120, 240]
    factor_names = ['std005', 'std010', 'std020', 'std060', 'std120', 'std240']
    items = zip(periods, factor_names)
    factor_list = []
    for n, name in items:
        # 核心计算
        data_pct = data.pct_change(1)
        std = data_pct.rolling(n).std()
        factors = std.stack().reset_index().rename(columns={0: 'factor_value'})
        # 补充信息
...
# 因子计算
def std_compute(data_raw, t1, factor_name, factor_type):
    # 数据处理
    data = std_datamanage(data_raw, t1)
    # 核心因子计算
    factors = std_keycompute(data, factor_name, factor_type, t1)
    return factors


# 因子
@print_execute_time
def std_sql_compute_sql(factor_name, factor_type, t1, t2, save, factor_table):
    # 数据读取
    data_raw = vpd_data_read(t1, t2, 13)
    # 因子计算
    factors = std_compute(data_raw, t1, factor_name, factor_type)
    # 因子存储
...
if __name__ == '__main__':
    ...
    #     print(t1, t2)
    #     std_sql_compute_sql('std', 'vpd', t1, t2, 1, 'fac_vpd_vol')

    fac_test = std_sql_compute_sql('std', 'vpd', '20181228', '20181228', 0, 'test')
